...
def map_payment_methods(conducto, policy_number=None):
    ...
    if 'DIRECTO' in c or 'AGENTE' in c:
        return 'spei', 'direct'
    # Default seguro
    pol_info = f" en póliza {policy_number}" if policy_number else ""
    logging.warning(f"Conducto de pago no reconocido: '{conducto}'{pol_info}. Se asignará por defecto a spei/direct.")
    return 'spei', 'direct'
    ...
    try:
        with engine.begin() as conn: # Inicia transacción SQL (Auto-commit al salir, rollback en excepciones)
            ...
            for poliza_num, group in grouped_vn:
                ...
                method_db, channel_db = map_payment_methods(conducto_final, clean_pol_num)
            ...
            for (pol_anterior, ano_vida), group in grouped_ren_vida:
                ...
                method_db, channel_db = map_payment_methods(latest_ren.get('Conducto de Pago', 'Directo'), clean_pol_num)
                ...
                if int(ano_vida) <= 1:
                    ...
                    if existing_ren:
                        ...
                        if new_pol_val is None:
                            ...
                            logging.warning(f"Número de renovación GMM duplicado detectado: {new_pol_val} (Padre: {clean_pol_num}). Se insertará con new_policy_number = NULL para conservar integridad y unicidad.")
                            new_pol_param = None
                        else:
